
# Cade Canavan — GIS Portfolio

B.S. Geography, Concentration in Sustainability | Minor in Philosophy — Salem State University, May 2025 | GPA: 3.7  
GIS Certificate | UAV/Drone Operations | Spatial Analysis & Data Visualization

Passionate about applying geospatial tools to solve real-world environmental and community challenges.

---

## Skills & Tools

**GIS & Spatial Analysis**  
![ArcGIS Pro](https://img.shields.io/badge/ArcGIS%20Pro-2C7AC3?style=flat&logo=esri&logoColor=white)
![ArcGIS Online](https://img.shields.io/badge/ArcGIS%20Online-2C7AC3?style=flat&logo=esri&logoColor=white)
![QGIS](https://img.shields.io/badge/QGIS-589632?style=flat&logo=qgis&logoColor=white)
![ArcGIS StoryMaps](https://img.shields.io/badge/StoryMaps-2C7AC3?style=flat&logo=esri&logoColor=white)  
Spatial Analysis (Vector & Raster) · Spatial Joins · Geocoding · Remote Sensing · NDVI Analysis · Canopy Height Modeling

**Remote Sensing & UAV**  
![UAV/Drone Operations](https://img.shields.io/badge/UAV%20%2F%20Drone%20Ops-555555?style=flat)  
Multispectral Imagery · Orthomosaic Generation · Image Segmentation

**Data & Design**  
![Excel](https://img.shields.io/badge/Excel-217346?style=flat&logo=microsoft-excel&logoColor=white)  
Data Aggregation & Visualization · Cartographic Design · Professional Writing

---

## Projects

### 1) UAV & AI Tree Canopy Health Analysis — Greenlawn Cemetery
Collected high-resolution multispectral UAV imagery and generated orthomosaics for spatial analysis of an urban cemetery. Applied AI-based image segmentation to delineate individual tree canopies, then produced NDVI rasters and a canopy height model to assess per-tree vegetation health. Extracted tree-level metrics to support urban tree inventory and management planning.

**Tools Used:** ArcGIS Pro · UAV/Multispectral Imagery · AI Segmentation · NDVI · Canopy Height Modeling

<!-- Poster (overview) -->
<img src="greenlawn_poster.png" alt="Greenlawn Cemetery UAV/NDVI/Segmentation poster overview" width="500"> <!-- Bar chart (results) --> <img src="greenlawn_barchart.png" alt="Greenlawn Cemetery tree health metrics bar chart" width="500">
</p>

---

### 2) Community Preservation Act StoryMap  
**City of Salem — Department of Planning and Community Development**

Geolocated and mapped 100+ projects funded through the Community Preservation Act using ArcGIS Pro. Built an interactive StoryMap integrating spatial data, project descriptions, and site images to improve public accessibility and engagement. Created a comprehensive, queryable spatial dataset with categorized thematic maps by project type.

**Tools Used:** ArcGIS Pro · ArcGIS Online · ArcGIS StoryMaps · Excel

**➡️ [View Project](https://storymaps.arcgis.com/stories/97e90b2559804cc8bdd1a731b5f95ab3)**

<img src="cpa_storymap.png" alt="Community Preservation Act StoryMap preview" width="700">

---

### 3) Food Access & Farmers Markets — Greater Boston, MA  
**Salem State University — GPH304**

Analyzed spatial relationships between USDA-designated Low Income, Low Access (LILA) census tracts and farmers market locations across Greater Boston. Joined demographic data from the 2020 Census to census tract boundaries using FIPS codes via Power Query, then calculated per-tract demographic percentages. Generated a near table to measure distance from each LILA census block to the nearest farmers market, produced distance maps using XY to Line, and visualized results through histograms and summary statistics.

**Tools Used:** ArcGIS Pro · Excel · Power Query · Census/TIGER Data · USDA Food Access Research Atlas

<p>
  <img src="farmersmarkets_map.png" alt="Greater Boston farmers markets map" width="500">
  <img src="farmersmarkets_methods.png" alt="Methods diagram for Food Access analysis" width="500">
</p>

---

## Contact
Open to GIS, sustainability, and environmental planning opportunities.  
**Email:** _add your email_ · **LinkedIn:** _add your link_

